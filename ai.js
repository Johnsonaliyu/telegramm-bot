...
const NVIDIA_MODEL = process.env.NVIDIA_MODEL || 'thinkingmachines/inkling';

const DESCRIPTION_SYSTEM_PROMPT = `You are a knowledgeable botanist writing short, friendly plant profiles for Flora Scan, a Telegram bot used in Nigeria. If you refer to the bot by name, always call it Flora Scan — never any other name.
...
const PLANT_QA_SYSTEM_PROMPT = `Your name is Flora Scan. This is fixed and non-negotiable — never call yourself by any other name (e.g. never "Bola Scan" or anything else), even if it feels like it would fit the Nigerian context better. If asked your name or "what are your capabilities", state clearly that you are Flora Scan.
You are Flora Scan, a friendly and knowledgeable agricultural assistant used by farmers, students, and gardeners — primarily in Nigeria.
...
// Strips <think>...</think> reasoning blocks that some reasoning models
// (Qwen 3.6 27B, Inkling) can still leak into message content as a
// defensive fallback, even when a provider-level "hide reasoning" flag is set.
...
async function callGroq(messages, maxTokens = 300) {
  ...
  const { data } = await axios.post(
    'https://api.groq.com/openai/v1/chat/completions',
    {
      ...
      // Qwen 3.6 27B is a reasoning model; without this its <think> content
      // gets returned inline as part of message.content.
...
async function callNvidia(messages, maxTokens = 300) {
  ...
  const { data } = await axios.post(
    'https://integrate.api.nvidia.com/v1/chat/completions',
    {
      ...
      temperature: 0.4,
      max_tokens: maxTokens,
      // Inkling is a switchable-reasoning model; NVIDIA NIM models take a
      // non-standard chat_template_kwargs flag (not reasoning_format) to
